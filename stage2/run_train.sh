...
# Generate per-task training configs
python3 _generate_train_configs.py

# Resolve base paths
BASE_DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$BASE_DIR"
CONFIG_DIR="$BASE_DIR/configs/train"
TARGET_JSON="$BASE_DIR/configs/train/train_target_tasks.json"
...
for task in $TARGETS; do
  ...
  (
    ...
    python "$TRAIN_SCRIPT" --config "$CONFIG_PATH" --mode train > "logs/bash_train/${CONFIG_NAME%.yaml}.log" 2>&1
...
wait
echo "All training jobs completed. See logs under logs/bash_train/*.log"
